...
class BVChallengeManager(ChallengeManager):
    ...
    def update_miner_scores(self, miner_commits: list[MinerChallengeCommit]) -> None:
        """
        ...
        for miner_commit in miner_commits:
            ...
            try:
                ...
                bt.logging.error(
                    ...
                    f"failed to get commit {miner_commit.encrypted_commit} scores and penalties: "
    ...
    def _ease_circle_in_out_shifted(self, x) -> float:
        x = x**1.5
        if x < 0.5:
            return 0.5 * (1 - math.sqrt(1 - (2 * x) ** 2))
        return 0.5 * (math.sqrt(1 - (2 * x - 2) ** 2) + 1)

    def _scaling_from_similarity(self, x) -> float:
    ...
    def _apply_softmax(self, scores) -> np.ndarray:
